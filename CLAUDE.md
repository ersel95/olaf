...
## Ürünler / hedefler
- **OlafCore** — UIKit'siz motor: `Olaf` facade, ring buffer, `BankingRedactor`, NDJSON disk persistans
...
- **OlafNetwork** — opsiyonel URLProtocol network capture; `.network` kategorisinde, redaksiyonlu.
...
## Değişmez kurallar
- **Redaksiyon opt-in** (`OlafConfiguration.redactionEnabled`, **default `false`**). Açıkken (PAN/IBAN/email/Authorization/Cookie/token) maskelenir; kapalıyken ham veri saklanır (`effectiveRedactor` → `NoopRedactor`). `bankingDefault` profili redaksiyonu açar — bankacılık/PII içeren akışlarda mutlaka `redactionEnabled: true` verilmeli.
